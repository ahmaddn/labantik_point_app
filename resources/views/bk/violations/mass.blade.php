    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16 font-semibold">Input Laporan Pelanggaran Massal</h5>
                </div>
            ...
            <div class="card border-none shadow-md rounded-xl bg-white dark:bg-zink-700">
                <div class="card-body p-4 sm:p-6">
                    <form action="{{ route('kesiswaan-bk.violations.mass.store') }}" method="POST" class="space-y-6">
                        @csrf
                        ...
                        <div class="pt-4 border-t border-slate-100 dark:border-zink-600 flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
                            <a href="{{ route('kesiswaan-bk.dashboard') }}" class="btn border border-slate-200 hover:bg-slate-50 dark:border-zink-600 dark:hover:bg-zink-600 text-slate-700 dark:text-zink-100 text-sm font-medium px-5 py-2.5 rounded-lg text-center">
                                Batal
                            </a>
...
@section('scripts')
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            // Map: id → data siswa (untuk siswa yang sudah dipilih)
                    ...
                    matched.forEach(function (row) {
                        ...
                        card.innerHTML = `
                            <div class="pt-0.5 shrink-0">
                                <input type="checkbox" class="result-checkbox size-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500 cursor-pointer"
                                    ${isSelected ? 'checked' : ''}>
                            </div>
                        ...
                        card.addEventListener('click', function (e) {
                            ...
                            toggleStudent(id, fullname, className, nis, card);
                        });
                        card.querySelector('.result-checkbox').addEventListener('change', function () {
                            toggleStudent(id, fullname, className, nis, card);
                        });
            ...
            function syncSelectedSection() {
                ...
                if (selectedMap.size === 0) {
                    ...
                    selectedMap.forEach(function (data, id) {
                        ...
                        tag.innerHTML = `${data.fullname}
                            <button type="button" class="hover:text-blue-900 dark:hover:text-blue-100 leading-none text-base font-bold" data-remove="${id}">&times;</button>`;
                        tag.querySelector('button').addEventListener('click', function () {
                            selectedMap.delete(id);
                            syncSelectedSection();
                            // update card di hasil pencarian jika masih tampil
                            const card = resultsContainer.querySelector(`[data-id="${id}"]`);
                            if (card) {
                                card.className = 'flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all duration-150 bg-white dark:bg-zink-700 border-slate-100 dark:border-zink-600 hover:border-blue-300 hover:shadow-sm';
                                card.querySelector('.result-checkbox').checked = false;
                            }
