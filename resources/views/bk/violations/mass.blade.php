        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            @if ($errors->any())
                <div class="relative mb-4 flex gap-3 rounded-md border border-red-200 bg-red-50 p-4 pr-12 text-sm text-red-700 dark:border-red-900/30 dark:bg-red-500/10 dark:text-red-400 shadow-sm">
                    ...
                    <button class="absolute top-4 right-4 text-red-400 hover:text-red-600 transition-colors duration-150" onclick="this.parentElement.style.display='none'">
            ...
            @if (session('success'))
                <div class="relative mb-4 flex gap-3 rounded-md border border-green-200 bg-green-50 p-4 pr-12 text-sm text-green-700 dark:border-green-900/30 dark:bg-green-500/10 dark:text-green-400 shadow-sm">
                    ...
                    <button class="absolute top-4 right-4 text-green-400 hover:text-green-600 transition-colors duration-150" onclick="this.parentElement.style.display='none'">
                        <i data-lucide="x" class="size-4"></i>
                    </button>
                </div>
            @endif
            ...
            @if (session('warning'))
                <div class="relative mb-4 flex gap-3 rounded-md border border-amber-200 bg-amber-50 p-4 pr-12 text-sm text-amber-700 dark:border-amber-900/30 dark:bg-amber-500/10 dark:text-amber-400 shadow-sm">
                    ...
                    <div class="grow">
                        ...
                        <p class="leading-relaxed">{{ session('warning') }}</p>
                    </div>
                    <button class="absolute top-4 right-4 text-amber-400 hover:text-amber-600 transition-colors duration-150" onclick="this.parentElement.style.display='none'">
                        <i data-lucide="x" class="size-4"></i>
                    </button>
                </div>
            @endif
            ...
            <div class="card">
                <div class="card-body">
                    <h6 class="text-15 mb-6">Form Input Pelanggaran Massal</h6>

                    <form action="{{ route('kesiswaan-bk.violations.mass.store') }}" method="POST">
                        @csrf

                        {{-- Step 1: Tanggal --}}
                        <div class="mb-6 pb-6 border-b border-slate-100 dark:border-zink-600">
                            <div class="flex items-center gap-3 mb-4">
                                <div class="flex items-center justify-center size-7 rounded-full bg-custom-500 text-white text-xs font-bold shrink-0">1</div>
                                <h6 class="text-14 font-semibold text-slate-700 dark:text-zink-100">Tanggal Kejadian Pelanggaran</h6>
                            </div>
                            <div class="flex flex-wrap gap-2 mb-3">
                                <button type="button" id="btn-today"
                                    class="btn bg-custom-500 border-custom-500 text-white hover:bg-custom-600 hover:border-custom-600 text-sm font-medium px-4 py-2">
                                    Hari Ini
                                </button>
                                <button type="button" id="btn-custom"
                                    class="btn bg-white border-slate-200 text-slate-700 dark:bg-zink-600 dark:border-zink-500 dark:text-zink-100 hover:bg-slate-50 dark:hover:bg-zink-500 text-sm font-medium px-4 py-2">
                                    Pilih Tanggal Lain
                                </button>
                            </div>
                        ...
                        {{-- Step 2: Pilih Pelanggaran --}}
                        <div class="mb-6 pb-6 border-b border-slate-100 dark:border-zink-600">
                            <div class="flex items-center gap-3 mb-4">
                                <div class="flex items-center justify-center size-7 rounded-full bg-custom-500 text-white text-xs font-bold shrink-0">2</div>
                                <h6 class="text-14 font-semibold text-slate-700 dark:text-zink-100">Pilih Jenis Pelanggaran</h6>
                            </div>
                        ...
                        <div class="mb-6 pb-6 border-b border-slate-100 dark:border-zink-600">
                            <div class="flex items-center gap-3 mb-4">
                                <div class="flex items-center justify-center size-7 rounded-full bg-custom-500 text-white text-xs font-bold shrink-0">3</div>
                                <h6 class="text-14 font-semibold text-slate-700 dark:text-zink-100">Pilih Siswa Pelanggar</h6>
                            </div>

                            <input type="text" id="search-student" placeholder="Cari nama, kelas, atau NIS siswa..." autocomplete="off"
                            ...
                            <div class="rounded-md border border-slate-200 dark:border-zink-500 overflow-hidden">
                                ...
                                {{-- Siswa terpilih --}}
                                <div id="selected-section" class="hidden border-t border-slate-200 dark:border-zink-500 bg-slate-50 dark:bg-zink-700/50 px-3 py-2.5">
                        ...
                        <div class="flex flex-col sm:flex-row sm:justify-end gap-3">
                            <a href="{{ route('kesiswaan-bk.dashboard') }}"
                                class="btn bg-white border-slate-200 text-slate-700 dark:bg-zink-600 dark:border-zink-500 dark:text-zink-200 hover:bg-slate-50 dark:hover:bg-zink-500 text-sm font-medium px-5 py-2.5 text-center">
                                Batal
                            </a>
                            <button type="submit"
                                class="btn bg-custom-500 border-custom-500 text-white hover:bg-custom-600 hover:border-custom-600 text-sm font-medium px-5 py-2.5">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    btnToday.addEventListener('click', function () {
        setActiveDate(btnToday, btnCustom);
        customDateContainer.classList.add('hidden');
        dateModeInput.value = 'today';
    });

    btnCustom.addEventListener('click', function () {
        setActiveDate(btnCustom, btnToday);
    ...
    function renderResults(query) {
        const matched = allRows.filter(r =>
            r.dataset.name.includes(query) ||
            r.dataset.nis.includes(query)  ||
            r.dataset.class.includes(query)
        );
        ...
        if (matched.length === 0) {
            ...
            matched.forEach(function (row) {
                ...
                card.className = 'flex items-start gap-3 p-3 rounded-md border cursor-pointer transition-all duration-150 '
                    + (isSelected
                        ? 'bg-custom-50 dark:bg-custom-900/20 border-custom-300 dark:border-custom-600'
                ...
                card.innerHTML = `
                    ...
                    <div class="flex flex-col gap-0.5 min-w-0">
                        ...
                        <span class="text-11 text-slate-400 dark:text-zink-400">NIS: ${nis}</span>
                    </div>`;
    ...
    function syncSelected() {
        ...
        selectedMap.forEach(function (data, id) {
            ...
            tag.querySelector('button').addEventListener('click', function () {
                ...
                if (c) {
                    c.className = 'flex items-start gap-3 p-3 rounded-md border cursor-pointer transition-all duration-150 bg-white dark:bg-zink-700 border-slate-100 dark:border-zink-600 hover:border-custom-300 hover:shadow-sm';
                    c.querySelector('.result-checkbox').checked = false;
                }
            });
